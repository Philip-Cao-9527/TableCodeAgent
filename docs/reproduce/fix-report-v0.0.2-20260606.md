...
- `scripts/run_demo_table_agent_smoke.sh`（历史旧入口，后续重构中移除）
- `src/tablecodeagent/benchmark/runner.py`（历史旧入口，已在 benchmark 口径重构中移除）
...
- `.tca/skills/growth-campaign-audit/agents/agent.yaml`
...
本报告中的 `benchmarks/runs/...` 属于 `v0.0.2` 当时的历史结果路径。后续 benchmark 口径重构后，新结果路径统一改为：

```text
benchmarks/results/<mode>/<YYYYMMDD-HHMMSS>__model-<model_name>__tasks-<task_id_or_group>/
```

本报告中的 `sandbox_code_agent` 是 runner 固定写入 `solve.py` 后验证 sandbox / pytest / `answer.json` 基础设施，不是 LLM 自主生成代码 benchmark。历史关键文件与作用如下：

- [task.json](/root/workspace/TableCodeAgent/benchmarks/tasks/growth_campaign_audit_001/task.json)：任务输入定义。
- [expected.json](/root/workspace/TableCodeAgent/benchmarks/tasks/growth_campaign_audit_001/expected.json)：外部校验期望，不应作为模型生成代码阶段输入。
- [tests/test_solution.py](/root/workspace/TableCodeAgent/benchmarks/tasks/growth_campaign_audit_001/tests/test_solution.py)：任务自带 pytest 校验脚本。
- [solve.py](/root/workspace/TableCodeAgent/benchmarks/runs/20260606-041838/workspaces/growth_campaign_audit_001.sandbox_code_agent/solve.py)：历史 runner 固定生成的代码文件，用于验证 sandbox 基础设施，不代表 LLM 生成。
- [answer.json](/root/workspace/TableCodeAgent/benchmarks/runs/20260606-041838/workspaces/growth_campaign_audit_001.sandbox_code_agent/answer.json)：历史固定 `solve.py` 执行后写出的答案文件。
- [results.jsonl](/root/workspace/TableCodeAgent/benchmarks/runs/20260606-041838/results.jsonl)：历史 benchmark 汇总结果。
- [traces 目录](/root/workspace/TableCodeAgent/benchmarks/runs/20260606-041838/traces/)：历史 trace 输出目录。

本报告中的 `optional_llm_agent` 调用了真实 API，但 prompt 明确要求“不修改任何文件”，因此它只验证真实模型工具调用与答案校验，不生成 `solve.py`。其历史证据路径为：

- [results.jsonl](/root/workspace/TableCodeAgent/benchmarks/runs/20260606-041843/results.jsonl)：历史真实 API tool-use demo 汇总结果。
- [demo_table_001.optional_llm_agent.json](/root/workspace/TableCodeAgent/benchmarks/runs/20260606-041843/traces/demo_table_001.optional_llm_agent.json)：历史真实 API tool-use trace。

历史验证过程中还暴露过 `pytest` 缺失导致 L2 sandbox 测试阶段失败或阻断后续 API demo 的问题。后续代码把 `pytest` 纳入测试依赖 preflight，失败时应记录为 `dependency_missing` 或 `dependency_install_failed`，不能伪装成通过。
...
- `optional_llm_agent` 当前不生成代码文件；它只验证真实 API 下模型是否发起表格工具调用并通过答案校验。
