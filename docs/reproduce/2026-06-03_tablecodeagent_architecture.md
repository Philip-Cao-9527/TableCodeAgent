...
- `tablecodeagent` 包已经包含表格工具层、答案校验层和 Agent 工具适配层。
- `load_table`、`profile_table`、`query_table`、`validate_answer` 已接入 `mini_claude` 的工具 schema 与 `execute_tool()` 分发路径。
- 已有不调用 LLM 的 smoke test 验证独立表格工具层和 Agent 工具注册闭环。
- tracing、benchmark runner、`run_python`、`run_tests`、数据集转换脚本尚未实现。
- 真实 LLM 端到端调用已提供可选脚本入口，但尚未作为稳定必过验证。
...
- `python/mini_claude/`：原 baseline 的 Python Coding Agent Runtime，是当前 Agent 主体。
- `python/tablecodeagent/`：本项目新增的表格任务能力层。
- `benchmarks/tasks/`：后续放表格任务样例、转换后的 benchmark 任务。
- `docs/reproduce/`：保存环境、复现、架构理解、实验记录。
- `src/`：原 baseline TypeScript 版，当前 MVP 第一阶段暂不作为主线。

## 3. baseline Python 架构

### 3.1 CLI 入口

文件：`python/mini_claude/__main__.py`
...
文件：`python/mini_claude/agent.py`
...
- 这表示 `execute_tool()` 的实际代码不在 `agent.py`，而是在 `python/mini_claude/tools.py`。
...
文件：`python/mini_claude/tools.py`
...
```text
1. 在 python/tablecodeagent/agent_tools.py 中集中定义 TABLE_TOOL_DEFINITIONS。
2. 在 python/tablecodeagent/agent_tools.py 中用 execute_table_tool() 适配真实表格函数。
3. 在 python/mini_claude/tools.py 中合并 TABLE_TOOL_DEFINITIONS 到 tool_definitions。
...
当前包装函数不直接写在 `tools.py`，而是集中放在 `python/tablecodeagent/agent_tools.py`，避免通用工具分发文件继续膨胀。适配层会把统一的工具输入字典转成具体函数参数，并把结果包装成统一 JSON：
...
- `python/tablecodeagent/` 是“工具真实能力”的实现位置。
...
文件：`python/mini_claude/prompt.py`
...
文件：`python/mini_claude/session.py`
...
文件：`python/tablecodeagent/table_tools/core.py`

Agent 适配文件：`python/tablecodeagent/agent_tools.py`
...
这些工具通过 `python/mini_claude/tools.py` 暴露给 Agent。`tools.py` 只负责 schema 合并和分发，真实表格能力仍在 `python/tablecodeagent/` 里。
...
文件：`python/tablecodeagent/validation/answer.py`
...
表格工具注册已经完成，下一步不建议继续扩大表格解析范围，而是优先补齐可复现评测闭环：

```text
已完成：表格工具注册进 mini_claude/tools.py
  -> 下一步：实现最小 trace logger
  -> 新增 demo benchmark runner
  -> 记录工具调用、输出、答案和是否通过
  -> 形成第一条可复现实验结果
```
...
1. 实现最小 trace logger，记录任务、工具调用、输出摘要、答案、校验结果和失败类型。
2. 新增本地 benchmark runner，先跑 `demo_table_001`，至少支持不调用 LLM 的 direct 工具执行和 Agent 工具分发 smoke。
3. 保留可选 LLM demo，但 API 缺失或模型不稳定时必须明确 `SKIP` 或“未验证”。
4. 再考虑 `run_python`、`run_tests` 和 WikiTQ/TabMWP/FinQA/TAT-QA 转换脚本。
