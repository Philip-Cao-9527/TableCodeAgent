...
- `src/tablecodeagent/workflows/credit_risk_scoring.py` 提供增强后的信贷风险评分 workflow，职责包括数据读取、贷前/贷后字段隔离、时间窗与标签窗口、重复申请与客户唯一性、缺失/异常/字段类型、特征处理说明、规则卡评分、业务规则校验、解释和 validation，不放入 `src/mini_claude/tools.py`，也不向真实 API benchmark prompt 暴露 helper。
- `src/tablecodeagent/workflows/finance_operations.py` 提供财务运营 workflow，面向 invoices/payments/customers/disputes/adjustments/policy 多表应收回款场景，覆盖对账、账龄、未核销现金、争议款、部分/超额/重复付款、credit limit、PO / 账期、ECL 与审计说明，同样不向真实 API benchmark 暴露 helper。
...
- `finance_operations_001` 已作为财务运营 fixture 覆盖多表对账、公开 contract、Pydantic answer schema、deterministic workflow、sandbox + pytest 回归与模拟 Agent 错误用例。
...
- `.tca/skills/*` 当前用于本项目的文档化 skill 资产与测试辅助说明，不代表 runtime 自动发现；如果要让 Agent 真正可见，还需要额外接入 `mini_claude/skills.py` 的发现路径。
