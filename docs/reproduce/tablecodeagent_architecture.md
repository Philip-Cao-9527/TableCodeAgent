...
- `v0.0.4` 起，真实 API benchmark 采用 no-helper 口径；runner 不向模型公开 `implementation_hints`、`allowed_project_helpers`、`solve_py_suggestion` 或 `build_*_report()` 这类项目 workflow helper。
- `task.json.output_contract` 使用 Pydantic answer model 作为公开结构契约来源；`real_api_code_agent` 会把 `model_json_schema()` 注入 prompt，并用同一 Pydantic model 校验 `answer.json`。
- `TRACE_VERSION` 当前为 `v0.0.4`；结果字段会区分 `benchmark_profile`、`helper_hints_exposed`、`schema_check.errors`、`run_python.exit_code`、stderr/stdout 摘要、`answer_file_saved`、`validation_pass_rate`、`test_pass_rate`、`tool_error_count`、`pytest_exit_code` 与 `pytest_failure_summary`。
- `src/tablecodeagent/workflow/` 提供产品态主 Agent Loop vertical slice，职责包括任务解析、表格发现、字段画像、上下文压缩、工具策略、候选代码沙箱执行、schema/pytest/validation 反馈、repair history、trace 归因和 report-scoped analysis memory；它通过 `run_table_product_workflow` tool 进入 MiniClaude 主链路，但在 no-helper benchmark 中被禁用。
- `tests/test_workflows/credit_risk_scoring.py`、`tests/test_workflows/growth_campaign_audit.py` 和 `tests/test_workflows/finance_operations.py` 是 helper-assisted oracle / fixture 回归资产，不再位于 `src/`，也不属于产品态主 Agent workflow；真实 API benchmark 不得 import 或调用这些 helper。
...
- `finance_operations_001` 已作为财务运营 fixture 覆盖多表对账、公开 contract、Pydantic answer schema、helper-assisted oracle、sandbox + pytest 回归与模拟 Agent 错误用例。
...
- `benchmarks/tasks/`：放表格任务样例、转换后的 benchmark 任务和公开 `output_contract`。
- `benchmarks/results/`：真实 API benchmark 输出目录；可提交人工确认后的关键结果证据，真实 key、`.env`、`configs/api/local/` 禁止提交。
- `tests/`：项目代码单元测试和集成测试，不代表真实 Agent benchmark；非 API 测试通过不能写成真实 LLM Agent 已通过。
...
- 当前仓库根目录已提供 `CLAUDE.md`，Agent 会自动获得项目级补充指令；缺失该文件只应写成配置缺口，不能写成已接入。
...
## 7. `.claude`、`CLAUDE.md` 与项目 skill 入口

结论：当前仓库已经直接新增根目录 `CLAUDE.md`，它是 MiniClaude 项目级长期总规则入口；`.claude/rules/*.md` 是主题补充规则；`.claude/skills/*/SKILL.md` 是具体场景 skill；`.codex/AGENTS.md` 只负责 Codex 协作和仓库开发治理。

从当前代码看：

- `prompt.py` 会向上查找 `CLAUDE.md`，并加载沿途目录的 `.claude/rules/*.md`。
- `skills.py` 会加载 `.claude/skills/*/SKILL.md`，并读取 `agents/agent.yaml` 元数据进入 system prompt。
- 根目录 `CLAUDE.md` 已覆盖 no-helper、API 成本、本地验证优先、三层 workflow 和 `.claude/skills/` 根目录规则。
- `.claude/skills/*` 是项目 skill 的正式根目录，`skills.py` 会自动发现项目 skill、解析 `SKILL.md` frontmatter，并读取 `agents/agent.yaml` 的展示、证据字段和边界元数据注入 system prompt；`.tca/skills/` 已删除，不再保留双轨兼容目录。
...
- `.claude/rules/*.md`：workflow 边界、no-helper 和验证规则的主题补充。
- `.codex/AGENTS.md`：Codex 协作和开发治理规则。
- `.claude/skills/*/SKILL.md`：具体业务场景 skill 说明、触发策略、输入输出和验证要求。

## 8. 下一步最小开发方向

表格工具注册、测试架构、真实 API benchmark 入口、公开输出契约、信贷风险评分 workflow 和财务运营 workflow 已经完成。下一步不建议继续扩大表格解析范围，而是持续扩展真实 API code agent benchmark 的任务覆盖和失败分析：

```text
已完成：表格工具注册进 mini_claude/tools.py
  -> 已完成：项目代码测试迁移到 tests/
  -> 已完成：真实 API benchmark 入口 benchmark_runner.py
  -> 已完成：Pydantic output_contract / schema_check.errors / pytest 型 validation 口径
  -> 已完成：no-helper real_api_code_agent benchmark 口径
  -> 已完成：增强 credit_risk_scoring_001 workflow fixture
  -> 已完成：新增 finance_operations_001 workflow / task / skill / schema / tests
  -> 下一步：持续扩展 real_api_code_agent 的任务覆盖和失败类型分析
  -> 形成更多可复现实验结果
```

优先级：

1. 扩展 `real_api_code_agent` 的真实 API 任务覆盖，记录 `benchmark_profile=no_helper`、`helper_hints_exposed=false`、`generated_code_path`、`answer_path`、`trace_path`、`workspace_path`、`schema_check.errors`、`run_python.exit_code`、`pytest_failure_summary` 和失败类型。
