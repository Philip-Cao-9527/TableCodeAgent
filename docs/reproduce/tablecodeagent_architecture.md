...
- `v0.0.3` 起，真实 API benchmark task 可以通过 `task.json.output_contract` 暴露公开输出契约；`real_api_code_agent` 会记录 `schema_check`、pytest 摘要和更细的失败类型。
- `TRACE_VERSION` 当前为 `v0.0.3`；结果字段会区分 `answer_file_saved`、`validation_pass_rate`、`test_pass_rate`、`tool_error_count`、`pytest_exit_code` 与 `pytest_failure_summary`。
- `src/tablecodeagent/workflows/credit_risk_scoring.py` 提供信贷风险评分 workflow，职责包括数据读取、质量检查、特征处理说明、规则卡评分、业务规则校验、解释和 validation，不放入 `src/mini_claude/tools.py`。
...
已完成：表格工具注册进 mini_claude/tools.py
  ...
  -> 已完成：output_contract / schema_check / pytest 型 validation 口径
  -> 已完成：credit_risk_scoring_001 workflow fixture
  -> 下一步：持续扩展 real_api_code_agent 的任务覆盖和重复测试分析
  -> 形成更多可复现实验结果
```

优先级：

1. 扩展 `real_api_code_agent` 的真实 API 任务覆盖，记录 `generated_code_path`、`answer_path`、`trace_path`、`workspace_path`、`schema_check`、`pytest_failure_summary` 和失败类型。
