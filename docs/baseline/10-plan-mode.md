...
Plan Mode 需要两个工具，标记为 `deferred`（延迟加载，详见[第 2 章](docs/baseline/02-tools.md)）：
...
Plan Mode 的 read-only 约束通过 `checkPermission()` 强制执行（详见[第 6 章](docs/baseline/06-permissions.md)）：
