...
- 生成的 review prompt 必须要求最终审查报告面向新手可读：技术结论、严重程度排序和证据链不能降低，但每条 finding 的“影响”“为什么这是问题”“最小修复建议”后面，应尽量补一小段准确的通俗解释，说明这件事在 TableCodeAgent 项目里到底会造成什么后果。
- 通俗解释要借鉴 `.agents/skills/knowledge-explainer/SKILL.md` 第二部分 `详细原理讲解（通俗版）` 的方法：从基础概念讲起、分层递进、把术语翻译成项目里发生了什么、掰开揉碎讲清楚；只借鉴讲法，不套用 3000 字等字数要求，也不要把 code review 报告写成超长教学文。
- 技术字段、JSON key、API 字段名、函数名和文件名可以保留英文原文，但不能连续堆成读者难以理解的大段句子。涉及多个字段、契约路径或 schema 路径时，优先用短列表或“中文业务含义 + 原字段名”的方式拆开，例如把 `data_quality.missing_po_invoice_ids` 解释为“缺少采购订单的发票编号清单”，再说明它和其他字段如何互相校验。
- 避免整段英文、长串专业名词或只由字段名串起来的解释。不能只写“字段没有对齐导致 schema / contract drift”，要补充类似“简单说，就是同一件业务事实在两个位置写法不一致，后续校验会不知道该信哪一个”的说明。
