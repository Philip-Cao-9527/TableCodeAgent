...
6. 如果本轮只是 skill/指令文件调整，最终说明按 `.codex/AGENTS.md` 不触发修复报告和版本号变更；若用户另行明确要求生成修复报告，报告内容质量也必须遵守 `.codex/AGENTS.md` 的通俗解释、多次测试标题分组和“先用三级标题建立多轮汇总区，再在其下使用 `#### 总表` 与 `#### 归因总结`”的层级要求。
