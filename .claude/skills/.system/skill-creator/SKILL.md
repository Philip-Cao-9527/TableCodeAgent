---
name: skill-creator
description: TableCodeAgent 项目内 skill 编写与审核规范。新增、修改或审核 .claude/skills/ 下的项目 skill 时使用；不用于修改全局系统 skill，也不代表完整自动创建 skill 系统已经实现。
---
...
本规范只约束 TableCodeAgent 仓库内 `.claude/skills/` 下的项目内 skill。它用于编写、更新和审核项目 skill 的结构、中文友好表达、边界、验证证据和失败处理。
...
8. 与仓库根目录 `.codex/AGENTS.md` 的关系是否清楚，是否遵守仓库级约束但未重复堆砌模板。
9. 与全局系统 skill 的边界是否清楚，是否避免把项目内规范误写成全局系统 skill 改造。
10. 若引用 `references/`，是否写清读取时机；`agents/agent.yaml` 是否存在且与 `SKILL.md` 内容一致。

## 与仓库级 AGENTS 的关系

`.codex/AGENTS.md` 只保留简洁索引规则：新增、修改或审核 `.claude/skills/` 必须遵守本规范。

不要在仓库级 AGENTS 中复制完整 skill 模板。后续新增或修改 `.claude/skills/` 时，以本规范为准。

## 与全局 `/root/.codex/skills/...` 的边界

- 全局系统 skill 只作为组织方式参考，不是本仓库可直接修改的目标文件。
- 本仓库任务默认只修改 `.claude/skills/` 下的项目内 skill 资产。
