...
- 报告要面向第一次接触本项目的新读者可读。涉及 `benchmark`、`trace`、`workspace`、`generated_code_path`、`answer.json`、`schema_check.errors`、Pydantic schema、task `output_contract`、no-helper、`SKIP`、`failure_type` 等英文术语或字段名时，可以保留原文，但必须在首次出现或关键结论处补一句中文解释，说明它在 TableCodeAgent 里代表什么、为什么影响结论。
- 不要频繁大段堆砌文件路径、字段名或英文缩写。证据路径必须精确，但正文解释应先讲清业务含义和评测影响，再列路径或字段；多个路径、字段或结果文件要用短列表、表格或“中文含义 + 原字段名”的方式拆开。
- 涉及现象解释、原因归因、失败类型或测试结论时，不能只写“某测试失败 / 某字段缺失 / schema 不匹配”。必须补充通俗说明：简单说发生了什么、为什么这会影响表格任务或 Agent 评测、用户或维护者接下来该如何理解这个结论。
- 如果本轮进行了多次真实 API 评测、代码冒烟测试、simulated Agent / mocked Agent 回归或其他重复验证，报告必须用清晰的 Markdown 标题区分每一次测试。推荐用平行的三级标题，例如 `### 第一次真实 API 评测`、`### 第二次本地 smoke 测试`、`### 第三次 simulated Agent 回归`；某一次测试下面的细节解释、失败原因、字段说明可用四级标题，例如 `#### 现象`、`#### 原因分析`、`#### 修复后变化`。
- 多次测试后必须给出与单次测试记录平行的汇总区，先使用三级标题，例如 `### 真实 API 多轮汇总`、`### 本地验证多轮汇总` 或 `### 多轮测试汇总`；在该三级标题下面再使用 `#### 总表` 汇总每次测试的命令、目标、结果、失败类型、证据路径和结论，并用 `#### 归因总结` 把反复失败或逐步收敛的原因讲清楚。不要直接在测试记录后孤立写 `#### 总表`，否则层级会和前面的测试结果记录不平行。总表不能只贴路径，必须让读者通过表格知道每一次测试到底证明了什么、没有证明什么。
- 真实 API、env 缺失、网络失败、模型未调用工具、权限不足或本地测试失败时，报告必须分析原因边界。无法确认根因时写“待确认”或“未验证”，不能把现象包装成已定位，也不能把 `SKIP` 写成通过。

生成 fix-report 时必须附可跳转的文件路径交叉引用，写清相关代码、测试、结果、trace、workspace、文档分别在哪里，便于直接打开查看。交叉引用必须优先落到具体文件名，不能只给文件夹名代替关键证据；benchmark、trace、workspace、generated code、answer、tests、docs 至少应给出可直接打开的文件级链接，目录链接只能作为辅助手段。链接格式必须使用标准 Markdown 相对路径，且相对当前 fix-report 文件书写；例如 `docs/reproduce/` 下的报告应写成 `[solve.py](../../benchmarks/.../solve.py)`、`[benchmark_runner.py](../../src/tablecodeagent/benchmark/benchmark_runner.py)`、`[README.md](../../README.md)`。能定位到行号时，链接目标必须使用 `#L{行号}` 锚点，例如 `[answer_models.py](../../src/tablecodeagent/benchmark/answer_models.py#L197)`；不要把 `:197` 写进 Markdown 链接目标，因为它会被当成文件名的一部分。不要写当前 IDE / Markdown 预览中无法直接跳转的绝对文件系统路径，例如 `/root/workspace/TableCodeAgent/...`。涉及 benchmark 或测试生成代码时，必须写清是否生成代码文件、生成代码文件路径、代码来源、代码文件用途、质量结论及依据。
...
3. 证据路径（截图、日志、trace、报告等）
   ...
   - 证据路径必须可复核；生成报告时使用可跳转 Markdown 相对路径交叉引用，优先落到具体文件名，能定位到行号时使用 `[文件名](相对路径#L行号)`，例如 `[answer_models.py](../../src/tablecodeagent/benchmark/answer_models.py#L197)`；不要写成 `[answer_models.py](../../src/tablecodeagent/benchmark/answer_models.py:197)`。
