...
description: TableCodeAgent 财务运营应收账款与现金回款 workflow 指导。需要设计、实现、审核或运行 invoices、payments、customers、disputes、policy 多表应收匹配、账龄、未核销、异常归因、客户风险和运营动作建议时使用；不用于暴露 benchmark 答案、项目 workflow helper、生产 ERP 对账系统或真实财务审计结论。
---
...
本 skill 用作财务运营 benchmark 场景和应收账款/现金回款 workflow 的流程指导。它描述通用业务检查顺序、输入输出边界、验证要求和失败处理；确定性计算应放在 `src/tablecodeagent/workflows/` 或相关表格工具中。

本 skill 不声明支持生产 ERP、银行流水自动核销、法定财务审计、税务申报、外汇折算、真实坏账准备计提或真实催收决策。

## 不适用范围

- 不用于给真实企业出具财务审计意见。
- 不用于替代会计系统、收款核销系统、授信管理或合规审批。
...
- 内部 deterministic workflow 只能用作 unit / integration / smoke / regression 测试。
...
- 报告中必须区分本地 deterministic workflow、模拟 Agent 测试和真实 API benchmark。
