...
- [ ] 将 `src/tablecodeagent/context/table_context.py` 的表格上下文压缩候选能力接入真实 Agent / benchmark 调用链：用字段语义、主键/关联键、时间窗口、质量标记、代表性样本预览和 trace 证据替代大表原文，并确保压缩时不丢失重复键、join 膨胀、缺失值、异常值和时间窗口错配等关键证据。当前 `load_table(path, preview_rows=5)` 只看前 5 行，无法代表全表分布，后续应改为结合分层/随机/首尾样本、字段统计、分位数、类别频次、缺失率、异常值和 group/join 分布摘要的上下文包。
...
- [ ] 评估将当前 `src/tablecodeagent/workflows/` 中的确定性参考实现迁移到 `tests/` 下的测试专用包，例如 `tests/test_workflows/`，并统一加 `test_` 前缀命名，避免它们被误解为产品态主 Agent workflow；迁移时同步更新 integration tests、simulated Agent tests、fixture oracle 引用和文档链接。
- [ ] 重新设计工业级主 Agent Loop workflow 能力，作为新的产品态模块，而不是复用当前 deterministic oracle：用户给 Agent 一个任务后，系统应自动完成任务解析、表格发现、字段画像、上下文压缩、工具选择、代码生成、沙箱执行、schema/pytest/validation 反馈、错误修正、trace 归因和 analysis memory 沉淀；该能力需要独立于 no-helper benchmark 口径设计，只能进行 product workflow，并明确区分 product workflow、helper-assisted workflow 和 no-helper capability evaluation。
- [ ] 将 `.tca/skills/<scenario>/` 项目 skill 正式接入 Agent Loop：当前 `src/mini_claude/skills.py` 只发现 `~/.claude/skills` 和项目 `.claude/skills`，不会自动识别 `.tca/skills/credit-risk-scoring`、`.tca/skills/growth-campaign-audit`、`.tca/skills/finance-operations`。后续需要统一 skill 根目录策略，支持 `.tca/skills` 的发现、system prompt 注入、`skill` tool 调用、`/<skill-name>` 手动触发、`agents/agent.yaml` 元数据读取和回归测试，确保场景 skill 真正参与主 Agent Loop。
- [ ] 补齐 MiniClaude 项目级指令入口：当前 `src/mini_claude/prompt.py` 已支持向上查找 `CLAUDE.md` 并加载 `.claude/rules/*.md`，但仓库主要长期规则仍在 `.codex/AGENTS.md`，场景规则又分散在 `.tca/skills`。后续需要决定是否新增根目录 `CLAUDE.md`、是否同步 no-helper / API 预算 / workflow 边界等关键规则，并补充加载与优先级测试，避免项目级指令只对 Codex 生效而 MiniClaude Agent 不可见。
- [ ] 补充表格问答与评测场景下的工作流。
...
- [ ] 将真实 API 复测前置约束写入 AGENTS.md：新增或修改 workflow / task contract 后，必须先补单元测试、集成测试和必要的模拟 Agent / edge case 回归；本地验证稳定后再跑真实 API benchmark，避免用真实 API 反复暴露可由本地测试提前发现的契约歧义，之前就出现了反复用api测试了八次的情况。
- [ ] 设计容器化Docker sandbox
