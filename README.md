...
- 真实 API benchmark：`real_api_code_agent` 调用真实 LLM API 生成 `solve.py`，再执行、校验并记录 trace。
- benchmark 输出契约：任务可在 `task.json.output_contract` 声明公开的 `answer.json` 顶层 schema，runner 会记录 `schema_check` 并区分 `answer_schema_mismatch` 与 `pytest_failed`。
...
- 信贷风险评分 workflow：新增 `credit_risk_scoring_001` fixture 与可复用 workflow，覆盖字段理解、数据质量、特征处理、规则卡评分、业务规则校验、解释和 validation。
- trace / validation 闭环：记录 API 调用、代码生成、sandbox 执行、schema 自检、测试校验、结果路径和失败类型。
...
└── benchmarks
    ...
    └── results                # 真实 API benchmark 输出结果；被 .gitignore 忽略
    ├── demo_table_001
    ├── excel_table_001
    ├── multi_table_001
    ├── multi_header_001
...
├── benchmarks/results/               # 真实 API benchmark 输出结果，禁止提交
...
该脚本不调用 LLM，不写入 benchmark 结果，只运行：
...
也可以把任务目录替换为 `benchmarks/tasks/credit_risk_scoring_001`，用于信贷风险评分 workflow 的真实 API 代码生成测试。

该脚本会调用本地 OpenAI-compatible API 配置，运行 `real_api_code_agent`。真实模型必须生成 `solve.py`，sandbox 执行后写出 `answer.json`，再由外部 pytest 或 validator 对照 `expected.json` 校验。
...
结果目录名必须包含时间、模型和任务信息。trace / result 至少记录 `api_called`、`model_name`、`generated_code_path`、`answer_path`、`code_generation_source`、`answer_file_saved`、`schema_check`、`pytest_exit_code`、`pytest_failure_summary`、`tool_error_count`、`validation.passed`、`skipped` 和 `failure_type`。如果 env、依赖、网络或 API 缺失，会记录为 `SKIP` 或失败，不伪装成功。
...
- [ ] 考虑Windows系统与linux系统的双向兼容问题  
- [ ] 在agents.md写入解决"当轮请求没有明确要求读取具体 env 文件导致无法自定义读取env文件，最后直接没有运行真实api测试就停止了的问题"的规则
- [ ] 在agents.md写入解决"每次都自动将benchmark/results文件夹放到.gitignore问题"的规则
- [ ] 现在随着任务数量增加，测试时间（营销增长和信贷风控）一轮需要四分钟左右，需要考虑工程优化
