...
- trace / validation 闭环：记录 API 调用、代码生成、sandbox 执行、测试校验、结果路径和失败类型。
...
结果目录名必须包含时间、模型和任务信息。trace / result 至少记录 `api_called`、`model_name`、`generated_code_path`、`answer_path`、`code_generation_source`、`validation.passed`、`skipped` 和 `failure_type`。如果 env、依赖、网络或 API 缺失，会记录为 `SKIP` 或失败，不伪装成功。
...
- [ ] 补充到agents.md 当中“在生成代码的时候文件命名一定要清晰具有区分度好理解”，这个可以放到顶层代码生成约束当中
